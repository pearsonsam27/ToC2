			    ...
			    `(or (and ,(visit a (not truth)) ,(visit b (not truth)))
				 (and ,(visit a truth) ,(visit b truth))))))
                     (:iff
                      (destructuring-bind (a b) args
			(if truth
			    `(and ,(visit '(:implies a b) truth)
				  ,(visit '(:implies b a) truth))
			    `(or ,(visit '(:implies a b) truth)
				  ,(visit '(:implies b a) truth)))))
                     (not
                      (assert (and args (null (cdr args))))
                      (visit (car args) (not truth)))
                     (and
		      ;; Demorgans might be broken for very long number of args
		      ;; Check later
		      (if truth
			  (cons op (mapcar #'visit
					args
					(make-list (length args)
						   :initial-element truth)))
			  (cons 'OR (mapcar #'visit
							 args
							 (make-list (length args)
								    :initial-element truth)))))
                     (or
...
(defun %dist-or-and-1 (literals and-exp)
  ...
  `(or ,@literals ,and-exp)
  `(and ,@(mapcar  (lambda (x)
		       (let ((lits (sort-vars `(,@(cdr x) ,@literals))))
		       `(OR ,@lits)))
       (cdr and-exp))))
...
(defun %dist-or-and-and (and-exp-1 and-exp-2)
  ...
  `(,@(reduce (lambda(acc x-prime)
		    (append acc (cdr x-prime)))
		    (map 'list (lambda(x);If starts to break through into let binding for reduce with sequeunce as (cdr (map..) :initial-value (car ..)
		       (%dist-or-and-1 (cdr x) and-exp-1))
			 (cdr and-exp-2)))))
...
Returns: (VALUES maxterms (LIST bindings-literals...))"
  ...
  (assert (every #'lit-p bindings))
  ;; HINT: use DPLL-BIND
  (let ((result (reduce (lambda(dpll bind)
	    (print "---")
	    (print dpll)
	    (print bind)
	    (destructuring-bind (pass-mt binded) dpll
	      (multiple-value-bind (new-mt acc-bind)
		  (dpll-bind pass-mt
			     bind
			     binded)
		  (print new-mt)
		  (print acc-bind)
		  (list new-mt acc-bind))))
	  bindings
	  :initial-value (list maxterms nil))))
...
Returns: (VALUES (OR T NIL) (LIST bindings-literals...))"
  ...
  (multiple-value-bind (maxterms bindings)
	...
	let((v (dpll-choose-literal(maxterms))))
	 (if dpll (multiple-value-bind((maxterms bindings),v))
		 (t
	         (dpll (multiple-terms-bindings((maxterms bindings), not v))))))
       ))))
